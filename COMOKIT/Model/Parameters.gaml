...
global {
	...
	string DEFAULT_CASE_STUDY_FOLDER_NAME <- "RK" const: true;
	...
	//simulation step
	float step<-1#h;
	date starting_date <- date([2020,6,23]);
	
	int num_infected_init <- 1; //number of infected individuals at the initialization of the simulation
	...
	float basic_viral_decrease <- 0.70; //Value to decrement the viral load in the environment
	...
	float male_ratio <- 48/52; // Ratio of 105 males for 100 females, see https://ourworldindata.org/gender-ratio
	...
	int work_hours_end_min <- 13; //ending working hour: min value
	int work_hours_end_max <- 20; //ending working hour: max value
	float schoolarship_rate <- 1.0; // The proportion of school agent
	int school_hours_begin_min <- 6; //beginning studying hour: min value
	int school_hours_begin_max <- 9; //beginning studying hour: max value
	int school_hours_end_min <- 12; //ending studying hour: min value
	int school_hours_end_max <- 19; //ending studying hour: max value
	int first_act_hour_non_working_min <- 6; //for non working day, min hour for the beginning of the first activity 
	int first_act_hour_non_working_max <- 11; //for non working day, max hour for the beginning of the first activity 
	int lunch_hours_min <- 11; //min hour for the begining of the lunch time
	int lunch_hours_max <- 14; //max hour for the begining of the lunch time
	...
	int min_age_for_evening_act <- 12; //min age of individual to have an activity after school
	float nb_activity_fellows_mean <- 3.0; 
	float nb_activity_fellows_std <- 2.0;

	int max_num_activity_for_non_working_day <- 3; //max number of activity for non working day
